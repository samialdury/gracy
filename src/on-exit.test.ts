...
describe('onExit', () => {
	...
	it('should register multiple functions', () => {
		const mockExitFn1 = vi.fn() // sync
		const mockExitFn2 = vi.fn().mockResolvedValue(null) // async
		...
		expect(mockExitFn1).not.toHaveBeenCalled()
		expect(mockExitFn2).not.toHaveBeenCalled()
	...
	it('should call registered functions on beforeExit', () => {
		const mockExitFn1 = vi.fn() // sync
		const mockExitFn2 = vi.fn().mockResolvedValue(null) // async
		...
		expect(mockExitFn1).toHaveBeenCalledTimes(1)
		expect(mockExitFn2).toHaveBeenCalledTimes(1)
		expect(mockLogger.debug).toHaveBeenNthCalledWith(
			3,
			{ code: 0 },
